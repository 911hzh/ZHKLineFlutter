  ...
  Widget build(BuildContext context) {
    return CustomKLineDemoShell(
      copy: const CustomDemoCopy(
        title: '自定义覆盖层 UI',
        description: '覆盖 buildOverlayView 可以替换默认指标标签、底部指标选择器，或叠加品牌水印、快捷操作和行情提示。',
...
class _CustomOverlayDelegate extends KLineDefaultDelegateImpl<KLineModel> {
  ...
  @override
  Widget? buildOverlayView(BuildContext context, KLineChartContext<KLineModel> chartContext) {
    final selected =
        chartContext.selectedNode?.item ??
        (chartContext.layoutNodes.isEmpty ? null : chartContext.layoutNodes.first.item);
    return Stack(
      children: [
        if (selected != null) _TopIndicatorPanel(context: chartContext, item: selected, adapter: adapter),
        Positioned(right: 12, top: 12, child: _RangeBadge(range: chartContext.visibleRange)),
        Positioned(left: 12, right: 12, bottom: 6, child: _FloatingIndicatorBar(context: chartContext)),
      ],
    );
  }
}
...
class _TopIndicatorPanel extends StatelessWidget {
  const _TopIndicatorPanel({required this.context, required this.item, required this.adapter});
      ...
      child: DecoratedBox(
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              const Text('自定义指标条', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700)),
              ...
              for (final entry in activeEntries)
                Padding(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(fontSize: 10, color: context.theme.indicatorColorAt(entry.colorIndex)),
                  ),
                ),
            ],
          ),
...
class _RangeBadge extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DecoratedBox(
      decoration: BoxDecoration(color: const Color(0xCC111827), borderRadius: BorderRadius.circular(14)),
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
        child: Text('visible ${range.start}-${range.end}', style: const TextStyle(color: Colors.white, fontSize: 10)),
      ),
    );
  }
}
            ...
            child: Row(
              children: [
                for (final indicator in [
                  ...
                  Expanded(
                    child: TextButton(
                      onPressed: () => this.context.controller.toggleIndicator(indicator.id),
                      child: Text(
                        indicator.label,
                        style: TextStyle(
                          fontSize: 11,
                          color: this.context.controller.activeIndicatorIds.contains(indicator.id)
                              ? Colors.blue
                              : Colors.grey,
                        ),
                      ),
