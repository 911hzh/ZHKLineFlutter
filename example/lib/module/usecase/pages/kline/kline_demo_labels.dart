        ...
        children: [
          ...
          if (active.contains(KLineTechnicalIndicatorType.ema.name))
            _row([
              ...
              ('EMA10', indicators.ema10, this.context.theme.indicatorColorAt(4)),
              ('EMA30', indicators.ema30, this.context.theme.indicatorColorAt(5)),
            ]),
          if (active.contains(KLineTechnicalIndicatorType.boll.name))
            _row([
              ('UPPER', indicators.bollUpper, this.context.theme.indicatorColorAt(0)),
              ('MB', indicators.bollMiddle, this.context.theme.indicatorColorAt(1)),
              ('LOWER', indicators.bollLower, this.context.theme.indicatorColorAt(2)),
            ]),
        ],
      ),
    );
    ...
    final children = values
        ...
        .map(
          (value) => Padding(
            ...
            child: Text('${value.$1}:${value.$2!.toStringAsFixed(2)}', style: TextStyle(color: value.$3, fontSize: 9)),
          ),
        )
        .toList();
...
class _SecondaryIndicatorLabels extends StatelessWidget {
  const _SecondaryIndicatorLabels({required this.context, required this.selected});
  ...
  Widget _label(KLineTechnicalIndicatorType type, KLineTechnicalIndicatorsModel indicators) {
    switch (type) {
      case KLineTechnicalIndicatorType.volume:
        return _row('VOL:', [('MA5', indicators.volumeMA5), ('MA10', indicators.volumeMA10)]);
      case KLineTechnicalIndicatorType.macd:
        return _row('MACD:', [('DIF', indicators.dif), ('DEA', indicators.dea), ('MACD', indicators.macd)]);
      case KLineTechnicalIndicatorType.kdj:
        return _row('KDJ:', [('K', indicators.k), ('D', indicators.d), ('J', indicators.j)]);
      case KLineTechnicalIndicatorType.rsi:
        return _row('RSI:', [('RSI6', indicators.rsi6), ('RSI12', indicators.rsi12), ('RSI24', indicators.rsi24)]);
      case KLineTechnicalIndicatorType.wr:
        return _row('WR:', [('WR6', indicators.wr6), ('WR10', indicators.wr10), ('WR14', indicators.wr14)]);
