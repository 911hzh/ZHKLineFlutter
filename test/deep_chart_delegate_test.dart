...
void main() {
  test('buildCumulativeDepthNodes accumulates bid and ask sizes independently', () {
    final nodes = DeepChartDefaultLayoutUtils.buildCumulativeDepthNodes(
      bids: const [DeepDepthEntry(price: 100, size: 2), DeepDepthEntry(price: 99, size: 3)],
      asks: const [DeepDepthEntry(price: 101, size: 4), DeepDepthEntry(price: 102, size: 5)],
    );
  ...
  test('axis labels align to grid lines and use a separate bottom row', () {
    ...
    final bottomLabels = DeepChartDefaultLayoutUtils.buildBottomPriceLabelLayouts(
      contentRect: contentRect,
      chartWidth: size.width,
    ...
    expect(bottomLabels[1].rect.center.dx, contentRect.left + contentRect.width / layout.gridVerticalCount);
    ...
    for (final label in bottomLabels) {
      ...
      expect(label.rect.height, layout.bottomHeight - layout.contentPadding.bottom);
    }
  });

  test('content rect reserves bottom label row even without bottom padding', () {
    const layout = DeepChartLayoutConfig(mainHeight: 80, contentPadding: EdgeInsets.fromLTRB(8, 0, 8, 0));
    final size = Size(160, layout.mainHeight + layout.bottomHeight);
    ...
    for (final label in bottomLabels) {
      expect(label.rect.bottom, lessThanOrEqualTo(size.height - layout.contentPadding.bottom));
    }
  });

  test('bottom label row leaves bottom padding below text area', () {
    const layout = DeepChartLayoutConfig(mainHeight: 120, bottomHeight: 24, contentPadding: EdgeInsets.only(bottom: 8));
  ...
  test('layout total height is chart height plus bottom label height', () {
    const layout = DeepChartLayoutConfig(mainHeight: 120, bottomHeight: 18, contentPadding: EdgeInsets.zero);
